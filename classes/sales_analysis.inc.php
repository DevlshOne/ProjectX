    ...
    class SalesAnalysis {
        ...
        function generateData($stime, $etime, $campaign_code, $agent_cluster_id, $combine_users, $user_group, $ignore_group, $vici_campaign_code = '', $ignore_arr = NULL, $vici_campaign_id = '') {
            ...
            $sql = "SELECT DISTINCT(agent_username), lead_tracking.vici_cluster_id FROM `lead_tracking` " .
                ...
                //		$sql_cluster.
                $sql_agent_cluster . $sql_campaign .

                // 		$sql_user_group.
                // 		$sql_user_group_for_activity_join.
                $sql_user_group_lmt .

                $sql_ignore_group_lmt . // 		$sql_ignore_group.
            ...
            foreach ($agent_array as $idx => $agentobj) {
                ...
                foreach ($agentobj->cluster_array as $cluster_id) {
                    ...
                    $sql = "SELECT SUM(amount) AS amount,count(id) AS salecount FROM sales " . $where .

                        (($combine_users) ? // GET USER AND USER2
                ...
                // TOTAL CALLZ!
                $sql = "SELECT COUNT(id) FROM lead_tracking " . " WHERE `time` BETWEEN '$stime' AND '$etime' " .
                ...
                $output_array[$ox++] = array(
                    ...
                    'agent_username' => $agentobj->username, 'cluster_id' => $agentobj->cluster_array,

                    'activity_paid' => $avtivity_paid, 'activity_wrkd' => $activity_worked, 'calls_today' => $num_total_calls_px, //$activity_num_calls,

                    'num_NI' => $num_NI, 'num_XFER' => $num_XFER,

                    'num_AnswerMachines' => $num_AnswerMachines,

                    'contacts_per_paid_hour' => $contacts_hr, 'calls_per_paid_hour' => $calls_hr,

                    'contacts_per_worked_hour' => $worked_contacts_hr, 'calls_per_worked_hour' => $worked_calls_hr,

                    'sale_cnt' => $running_salecnt, 'closing_percent' => $closing_percent, 'conversion_percent' => $conversion_percent, 'yes2all_percent' => $yes2all, 'sales_total' => $running_amount, 'paid_sales_total' => $running_paid_amount, 'paid_sale_cnt' => $running_paid_salecnt, 'avg_sale' => $avg_sale,

                    'paid_hr' => $paid_hr, 'wrkd_hr' => $wrkd_hr,);
            ...
            $totals = array(
                ...
                // ADDED IN THE LOOP
                'total_activity_paid_hrs' => $total_paid_hrs, 'total_activity_wrkd_hrs' => $total_active_hrs, 'total_calls' => $total_calls, 'total_NI' => $total_ni, 'total_XFER' => $total_xfer,

                'total_AnswerMachines' => $total_AnswerMachines,

                'total_contacts_per_paid_hour' => $total_contacts_hr, 'total_calls_per_paid_hour' => $total_calls_hr,

                'total_contacts_per_worked_hour' => $total_worked_contacts_hr, 'total_calls_per_worked_hour' => $total_worked_calls_hr,

                'total_sale_cnt' => $total_sale_cnt, 'total_sales' => $total_amount,

                'total_paid_sale_cnt' => $total_paid_sale_cnt, 'total_paid_sales' => $total_paid_amount,
