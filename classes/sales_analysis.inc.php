<?php
    ...
    class SalesAnalysis {
        ...
        function generateData($stime, $etime, $campaign_code, $agent_cluster_id, $user_team_id, $combine_users, $user_group, $ignore_group, $vici_campaign_code = '', $ignore_arr = NULL, $vici_campaign_id = '') {
            ...
            $where = " WHERE sale_time BETWEEN '$stime' AND '$etime' " . // EXCLUDE ANYTHING ROUSTING RELATED
                " AND `is_paid` != 'roustedcc' " . //	" AND `account_id`='".$_SESSION['account']['id']."' ".
            ...
            $agent_array = array();
            $cluster_array = array();
            $sql = "SELECT DISTINCT(agent_username), lead_tracking.vici_cluster_id FROM `lead_tracking` " . "WHERE lead_tracking.`time` BETWEEN '$stime' AND '$etime' " . $sql_vici_campaign . //		$sql_cluster.
                $sql_agent_cluster . $sql_campaign . // 		$sql_user_group.
                // 		$sql_user_group_for_activity_join.
                $sql_user_group_lmt . $sql_ignore_group_lmt . // 		$sql_ignore_group.
                $ofcsql . " ORDER BY agent_username ASC";
            "" . "" . "";
            echo $sql . "<br />\n";
...
//                        echo "Skipping " . $tmp . " --> not in selected team." . PHP_EOL;
            ...
            foreach ($agent_array as $idx => $agentobj) {
                ...
                foreach ($agentobj->cluster_array as $cluster_id) {
                    $sql = "SELECT SUM(amount) AS amount,count(id) AS salecount FROM sales " . $where . (($combine_users) ? // GET USER AND USER2
                            " AND (agent_username IN ('" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "','" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "2') ) " : // ELSE JUST GET THE SPECIFIED USER
                            " AND agent_username='" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "' ") . " AND agent_cluster_id='$cluster_id' ";
                    //	echo "\n".$sql."\n";
                ...
                //echo $amount.' '.$salecnt."\n";
                // TOTAL CALLZ!
                $sql = "SELECT COUNT(id) FROM lead_tracking " . " WHERE `time` BETWEEN '$stime' AND '$etime' " . // EXCLUDE ANYTHING ROUSTING RELATED
                    " AND dispo != 'SALECC' " . //	" AND `account_id`='".$_SESSION['account']['id']."' ".
                    " AND lead_id > 0 " . (($combine_users) ? // GET USER AND USER2
                        " AND (agent_username IN ('" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "','" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "2') ) " : // ELSE JUST GET THE SPECIFIED USER
                        " AND agent_username='" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "' ") . $ofcsql . (($campaign_id) ? " AND campaign_id='" . $campaign_id . "' " : "") . $sql_agent_cluster . $sql_vici_campaign;
                //echo $sql."<br />\n\n";
                list($num_total_calls_px) = $_SESSION['dbapi']->ROqueryROW($sql);
                $sql = "SELECT COUNT(id) FROM lead_tracking " . //									" FORCE INDEX (time) ".
                    //" USE INDEX (time) ".
                    " WHERE `time` BETWEEN '$stime' AND '$etime' " . //	" AND `account_id`='".$_SESSION['account']['id']."' ".
                    // EXCLUDE ANYTHING ROUSTING RELATED
                    " AND dispo != 'SALECC' " . " AND lead_id > 0 " . (($combine_users) ? // GET USER AND USER2
                        " AND (agent_username IN('" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "','" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "2') )" : // ELSE JUST GET THE SPECIFIED USER
                        " AND agent_username='" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "' ") . " AND (`dispo`='NI') " . $ofcsql . (($campaign_id) ? " AND campaign_id='" . $campaign_id . "' " : "") . $sql_agent_cluster . $sql_vici_campaign;
                //echo "\n".$sql."\n";
                ...
                $sql = "SELECT COUNT(id) FROM lead_tracking " . //									" FORCE INDEX (time) ".
                    ...
                    " AND dispo != 'SALECC' " . " AND lead_id > 0 " . (($combine_users) ? // GET USER AND USER2
                        " AND (agent_username IN ('" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "','" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "2') ) " : // ELSE JUST GET THE SPECIFIED USER
                        " AND agent_username='" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "' ") . " AND (`dispo`='A') " . $ofcsql . (($campaign_id) ? " AND campaign_id='" . $campaign_id . "' " : "") . $sql_agent_cluster . $sql_vici_campaign;
                //echo "\n".$sql."\n";
                // ANSWERING MACHINE STATS
                ...
                $sql = "SELECT COUNT(id) FROM transfers " . " WHERE xfer_time BETWEEN '$stime' AND '$etime' " . //	" AND `account_id`='".$_SESSION['account']['id']."' ".
                    " AND (verifier_dispo IS NOT NULL  AND verifier_dispo != 'DROP' AND `verifier_dispo` != 'SALECC') " . $ofcsql . (($combine_users) ? // GET USER AND USER2
                        " AND (agent_username IN ('" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "','" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "2') ) " : // ELSE JUST GET THE SPECIFIED USER
                        " AND agent_username='" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "' ") . (($campaign_id > 0) ? " AND campaign_id='" . intval($campaign_id) . "' " : "") . $sql_vici_campaign;
                //			echo $sql."<br />\n";
                ...
                if ($combine_users) {
                    list($activity_paid, $activity_wrkd, $activity_num_calls) = $_SESSION['dbapi']->ROqueryROW("SELECT SUM(paid_time), SUM(activity_time),SUM(calls_today) FROM activity_log " . "WHERE `time_started` BETWEEN '$stime' AND '$etime' " . //	" AND `account_id`='".$_SESSION['account']['id']."' ".
                    ...
                    list($activity_paid2, $activity_wrkd2, $activity_num_calls2) = $_SESSION['dbapi']->ROqueryROW("SELECT SUM(paid_time), SUM(activity_time),SUM(calls_today) FROM activity_log " . "WHERE `time_started` BETWEEN '$stime' AND '$etime' " . //	" AND `account_id`='".$_SESSION['account']['id']."' ".
                    ...
                    list($activity_paid, $activity_wrkd, $activity_num_calls) = $_SESSION['dbapi']->ROqueryROW("SELECT SUM(paid_time), SUM(activity_time),SUM(calls_today) FROM activity_log " . "WHERE `time_started` BETWEEN '$stime' AND '$etime' " . //" AND `account_id`='".$_SESSION['account']['id']."' ".
                        " AND `username`='" . mysqli_real_escape_string($_SESSION['db'], strtolower($agentobj->username)) . "' " . //" AND `vici_cluster_id`='".$cluster_array[$idx]."' ".
                        (($campaign_code) ? " AND campaign='" . mysqli_real_escape_string($_SESSION['db'], $campaign_code) . "' " : ""));
                }
                /// $activity_num_calls
                ...
                $output_array[$ox++] = array('agent_username' => $agentobj->username, 'cluster_id' => $agentobj->cluster_array, 'activity_paid' => $avtivity_paid, 'activity_wrkd' => $activity_worked, 'calls_today' => $num_total_calls_px, //$activity_num_calls,
                    'num_NI' => $num_NI, 'num_XFER' => $num_XFER, 'num_AnswerMachines' => $num_AnswerMachines, 'contacts_per_paid_hour' => $contacts_hr, 'calls_per_paid_hour' => $calls_hr, 'contacts_per_worked_hour' => $worked_contacts_hr, 'calls_per_worked_hour' => $worked_calls_hr, 'sale_cnt' => $running_salecnt, 'closing_percent' => $closing_percent, 'conversion_percent' => $conversion_percent, 'yes2all_percent' => $yes2all, 'sales_total' => $running_amount, 'paid_sales_total' => $running_paid_amount, 'paid_sale_cnt' => $running_paid_salecnt, 'avg_sale' => $avg_sale, 'paid_hr' => $paid_hr, 'wrkd_hr' => $wrkd_hr,);
                // TOTALS ADDUP
            ...
            $totals = array(// ADDED IN THE LOOP
                'total_activity_paid_hrs' => $total_paid_hrs, 'total_activity_wrkd_hrs' => $total_active_hrs, 'total_calls' => $total_calls, 'total_NI' => $total_ni, 'total_XFER' => $total_xfer, 'total_AnswerMachines' => $total_AnswerMachines, 'total_contacts_per_paid_hour' => $total_contacts_hr, 'total_calls_per_paid_hour' => $total_calls_hr, 'total_contacts_per_worked_hour' => $total_worked_contacts_hr, 'total_calls_per_worked_hour' => $total_worked_calls_hr, 'total_sale_cnt' => $total_sale_cnt, 'total_sales' => $total_amount, 'total_paid_sale_cnt' => $total_paid_sale_cnt, 'total_paid_sales' => $total_paid_amount, // MATH GENERATED FROM ABOVE DATA
                'total_closing' => $total_closing, 'total_conversion' => $total_conversion, 'total_yes2all' => $total_yes2all, 'total_avg' => $total_avg, 'total_paid_hr' => $total_paid_hr, 'total_wrkd_hr' => $total_wrkd_hr,);
            //print_r($totals);
            ...
            if (!isset($_REQUEST['no_nav'])) {
                ...
                <form id="saleanal_report" method="POST" action="<?= $_SERVER['PHP_SELF'] ?>?area=sales_analysis&no_script=1" onsubmit="return genReport(this, 'sales')">
                    ...
                    <table border="0" width="100%">
                        <tr>
                            <td height="40" class="pad_left ui-widget-header">Daily Sales Analysis Report</td>
                        </tr>
                        <tr>
                            <td colspan="2">
                                <script>
...
//            print_r(func_get_args());
        ...
        public function sendReportEmails() {
            ...
            $ignore_group = NULL;
            connectPXDB();
            $res = $_SESSION['dbapi']->query("SELECT * FROM report_emails " . " WHERE enabled='yes' ");
            ...
            while ($row = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
                ...
                $headers = array("From" => "ATC Reporting <support@advancedtci.com>", "Subject" => $subject, "X-Mailer" => "ATC Reporting System", "Reply-To" => "ATC Reporting <support@advancedtci.com>");
