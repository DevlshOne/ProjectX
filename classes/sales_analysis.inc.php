...
class SalesAnalysis
{
    ...
    function generateData($stime, $etime, $campaign_code, $agent_cluster_id, $user_team_id, $combine_users, $user_group, $ignore_group, $vici_campaign_code = '', $ignore_arr = NULL, $vici_campaign_id = '', $output_method = 'report')
    {
        ...
        if ($vici_campaign_id) {
            ...
            //$sql_vici_campaign_xfers .= " AND `vici_campaign_id`='".mysqli_real_escape_string($_SESSION['db'],$vici_campaign_id)."' ";
        }
        ...
        foreach ($agent_array as $idx => $agentobj) {
            ...
            $sql = "SELECT COUNT(transfers.id) FROM `transfers` " .
                (($sql_vici_campaign != '') ? " LEFT JOIN `lead_tracking` ON `lead_tracking`.id=`transfers`.lead_tracking_id " : "") .
                " WHERE `transfers`.xfer_time BETWEEN '$stime' AND '$etime' " .
                //	" AND `account_id`='".$_SESSION['account']['id']."' ".
                " AND (`transfers`.verifier_dispo IS NOT NULL  AND `transfers`.verifier_dispo != 'DROP' AND `transfers`.`verifier_dispo` != 'SALECC') " .
                $ofcsql .
                (($combine_users) ?
                    // GET USER AND USER2
                    " AND (`transfers`.agent_username IN ('" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "','" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "2') ) "
                    :
                    // ELSE JUST GET THE SPECIFIED USER
                    " AND `transfers`.agent_username='" . mysqli_real_escape_string($_SESSION['db'], $agentobj->username) . "' "
                ) .
                (($campaign_id > 0) ? " AND `transfers`.campaign_id='" . intval($campaign_id) . "' " : "") .
                $sql_vici_campaign;
    ...
    function renderGenerateDataXML($results_array = array(), $totals_array = array())
    {
