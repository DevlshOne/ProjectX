...
//print_r($call_group);
	...
	//
						...
						if($x++ == 0)$source_user_group_sql = " AND (";
						else		$source_user_group_sql .= " OR ";
...
//echo '<br />';
			...
			$sql = "SELECT COUNT(`id`) FROM `sales` ".
								...
								//	" AND `account_id`='".$_SESSION['account']['id']."' ".
									...
									" AND (agent_cluster_id='$cluster_id' OR verifier_cluster_id='$cluster_id') ".
...
//			echo $sql."\n";
			...
			list($paid_sales_cnt) = queryROW($sql." AND `is_paid`='yes' ");
			...
			$sql = "SELECT SUM(amount) FROM `sales` ".
								...
								//	" AND `account_id`='".$_SESSION['account']['id']."' ".
									...
									(($ignore_source_cluster_id > 0)?" AND agent_cluster_id != '$ignore_source_cluster_id' ":'').
									$source_user_group_sql.
									" AND `is_paid`='yes' ";//" AND `call_group`='".mysqli_real_escape_string($_SESSION['db'],$call_group)."'";
			//echo $sql."\n";
			...
			$sql = "SELECT SUM(agent_amount),SUM(verifier_amount) FROM `transfers` ".
						$xfer_where.
						" AND (verifier_dispo='PAIDCC') "; //verifier_dispo='SALE' OR
...
//			}
			...
			$sql = "SELECT * FROM `vicidial_agent_log_archive` ".
			      " WHERE `user` = '".mysqli_real_escape_string($_SESSION['db'],$username)."' ".
