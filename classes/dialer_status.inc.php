    ...
    function displayDialers()
    {
        ...
        <div class="block">
            <div class="block-header bg-primary-light">
                <h4 class="block-title">Dialer Status Dashboard</h4>
                <button id="clusterSelectButton" class="btn btn-sm btn-primary">Add Cluster Tile</button>
                <button id="refreshRateButton" class="btn btn-sm btn-primary">Change Refresh [40]</button>
                <button id="stopDialersButton" class="btn btn-sm btn-danger">Stop All Dialing</button>
                <button id="forceHopperButton" class="btn btn-sm btn-danger">Force Hopper</button>
                <button id="switchContrast" class="btn btn-sm btn-dark" value="Dark Mode" onclick="">Dark Mode</button>
            </div>
        ...
        <script>
            ...
            $('#dialerStatusZone').ready(function () {
                    ...
                    buttons: {
                        'Save': function () {
                            $('#clusterSelection option:selected').each(function () {
                                ...
                                tileDefs.push(new clusterDef(this.value, clusterData.type, clusterData.name, clusterData.ip, new Array('ALL-ACTIVE'), new Array('ALL-GROUPS')));
                            });
                            if (frontEnd_debug) {
                ...
                $('#dialog-modal-cluster-filters').dialog({
                    ...
                    buttons: {
                        'Save': function (e) {
                            ...
                            saveUserPrefs();
                            if (frontEnd_debug) {
                                console.log('Cluster filters have just been changed on tile ' + tileID + ' :: ', tileDefs);
                            }
                            $(this).dialog('close');
                        },
                        'Cancel': function () {
                            ...
                            if (prefs.length) {
                                ...
                                $.each(tileDefs, function (i, v) {
                                    if (v.name === undefined || v.name === '') {
                                        ...
                                        oldPrefsData = true;
                                        let clusterData = getClusterInfoByClusterID(v.cluster_id)[0];
                                        tileDefs[i] = new clusterDef(v.cluster_id, clusterData.type, clusterData.name, clusterData.ip, v.groups, v.user_group_filter);
                                    }
                                });
                ...
                $('#dialerStatusZone').on('click', '.selectFiltersButton', function () {
                    ...
                    let clusterData = getClusterInfoByClusterID(tileDefs[tileID].cluster_id)[0];
                    ...
                    $.each(clusterData['campaign_options'], function (i, v) {
                    ...
                    $.each(clusterData['usergroup_options'], function (i, v) {
                ...
                function parseTable(tile_id, tbl) {
                    if (frontEnd_debug) {
                        console.log('Parsing data for Tile ID :: ', tile_id);
                    }
                    ...
                    if (tdValues.length > 1) {
                        ...
                        $newLayout.append('<tr style="height:35px;vertical-align:bottom;"><td colspan="2" class="pct100 align_center"><button title="Select Filters for this Cluster" id="selectClusterFilters_' + tile_id + '" class="selectFiltersButton btn btn-sm btn-primary">Filters</button><button title="Load in ViciDial" id="loadCluster_' + tile_id + '" class="loadClusterButton btn btn-sm btn-success"><a target="_blank" href="http://' + clusterInfo[tile_id]['ip'] + '/vicidial/admin.php' + loadClusterAssessment(tile_id) + '">Load</a></button><button title="View Cluster Details" class="btn btn-sm btn-success"><a target="_blank" href="http://' + tileDefs[tile_id].web_ip + '/vicidial/realtime_report.php">Details<a></button></td></tr>');
                        $newLayout.append('<tr style="height:35px;vertical-align:bottom;"><td colspan="2" class="pct100 align_center"><button title="Stop Dialing for this Cluster" id="stopDialersButton_' + tile_id + '" class="stopDialersButton btn btn-sm btn-danger">Stop Dialer</button><button title="Force Hopper Reset for this Cluster" class="forceHopperButton btn btn-sm btn-danger" id="forceHopperButton_' + tile_id + '">Force Hopper</button></td></tr>');
                    } else {
                ...
                function parseDialerStatusData(tileID, dialerStatusData) {
                    ...
                    if (frontEnd_debug) {
                        console.log('Rebuilding Tile ID :: ', tileID);
                    }
                ...
                function getDialerStatusData() {
                    ...
                    $.each(tileDefs, function (i, v) {
                        ...
                        $.ajax({
                            ...
                            crossDomain: false,
                            crossOrigin: false,
                            url: 'api/api.php?get=dialer_status&mode=json&action=getClusterDataByUserPrefs&t=' + i.toString(),
