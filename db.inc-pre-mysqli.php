...
//    $db	= $_SESSION['db'] = mysql_connect($_SESSION['site_config']['sqlhost'], $_SESSION['site_config']['sqllogin'], $_SESSION['site_config']['sqlpass']) or die(mysql_error()."Connection to MySQL Failed.");
//	mysql_select_db($_SESSION['site_config']['sqldb'], $db) or die("Could not select database ".$_SESSION['site_config']['sqldb']);
	...
	function query($cmd, $mode=0){			# with mode = 0 or 1, it will return the result set, all the records returned.
		##print $cmd."<br>";
		$res = mysql_query($cmd, $_SESSION['db']);
		if(!$mode || $mode == 1){
			return $res;
		}else if($mode == 2){
			return (mysql_num_rows($res) > 0)?mysql_fetch_row($res):null;
		}else if($mode == 3){
	...
	function execSQL($cmd){
		mysql_query($cmd, $_SESSION['db']) or die("Error in execSQL(".$cmd."):".mysql_error());
		if(($cnt=mysql_affected_rows()) > 0)return $cnt;
		else	return 0;
	}
	/***************************************/
	 ...
	 function buildWhereFromArray($tmpAr, $field,$restrict){
		...
		if($tmpAr && !in_array($restrict, $tmpAr)){
			...
			for($x=0;$x<$lifecnt;$x++){
				...
				$where .= "".$field." = '".mysql_real_escape_string($tmpAr[$x])."' ";
			}
			$where.=")";
		}
		return $where;
	...
	/***************************************/
	 function buildWhereInList($tmpAr, $field,$restrict){
		...
		if($tmpAr && !in_array($restrict, $tmpAr)){
			$lifecnt = count($tmpAr);
			for($x=0;$x<$lifecnt;$x++){
				$inlist.="'".mysql_real_escape_string($tmpAr[$x])."'";
				if($x+1 < $lifecnt){
					$inlist.=",";
				}
			}
