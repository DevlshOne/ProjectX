...
function postToPXServer($px_ip, $callid, $channel, $vici_ip){

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"http://".$px_ip.":2288/HangingUpCall?call_id=".urlencode($callid)."&channel=".urlencode($channel)."&vici_server_ip=".urlencode($vici_ip));
...
function postToPXServerByExt($px_ip, $viciext, $vici_ip){

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,"http://".$px_ip.":2288/HangingUpCall?vici_extension=".$viciext."&vici_server_ip=".$vici_ip);
...
// DETECT THE IP ADDRESS, BASED ON THE LOCAL MACHINES IP ADDRESS/SUBNET/A.B.C.x
	...
	// VERIFIERS - HAVE TO SEND TO BOTH PX SERVERS BECAUSE EXTENSIONS ARE SCATTERED
	...
	## DEBUGGING LOG
	if($file_debug){
		$fh = fopen("/tmp/px-verifiers-server-relay.log", "a");
...
*/
	...
	if( $agivars['agi_type'] == 'SIP' || $agivars['agi_type'] == 'IAX2'){ //preg_match("/SIP\/opensip/", $agivars['agi_channel']) ){
		...
		foreach($px_server_ip as $px_ip){

			if($file_debug){
				fwrite($fh, "Posting Call ID Hangup to PX ($px_ip): ".$callid." Dialer:".$ipaddr."\n");
			}

			postToPXServer($px_ip, $callid, $agivars['agi_channel'], $ipaddr);
		}
...
*/
		...
		foreach($px_server_ip as $px_ip){
			...
			if($file_debug){
				fwrite($fh, "Posting Hangup to PX ($px_ip): ".$viciext." Dialer:".$ipaddr."\n");
			}

			postToPXServerByExt($px_ip, $viciext, $ipaddr);
		}
