...
/// DO STUFF
			...
			foreach($id_array as $idx=>$activity_id){
				...
				$dat = array();
				$dat['paid_time'] = floatval($hours) * 60;
				$dat['notes'] = $notes;
...
//			}
			...
			if(trim(strtolower($_REQUEST['report_mode'])) == "true"){

				$dat['report_mode'] = true;
		...
		if($_SESSION['api']->mode != "csv"){
			...
			if(isset($_REQUEST['index']) && isset($_REQUEST['pagesize'])){
				...
				list($totalcount) = mysql_fetch_row($_SESSION['dbapi']->employee_hours->getResults($cntdat));
				...
				if($dat['report_mode'] == true){
					...
					while($row = mysql_fetch_array($res, MYSQL_ASSOC)){
						...
						$sql = "SELECT `id`,`verifier_dispo_time`,`time` FROM `lead_tracking` ".
															"WHERE `verifier_username`='".addslashes( $user )."' ".
					...
					$out = "Date,Agent,Group,Office,Activity,Activity(New),Activity(New-Min),In-Call Time,In-Call Time(Min),Handle Time,Handle Time(Min),Ready Time,Queue Time,Paused Time,Paid Hours,Notes\r\n";
					...
					$out .= "TOTALS,".count($rowarr).",,,".round($total_activity/60,2).",".renderTimeFormattedSTD($total_act).",".round($total_act/60,2).",".
							renderTimeFormattedSTD($t_incall).",".round($t_incall/60,1).",".
							renderTimeFormattedSTD($t_handle).",".round($t_handle/60,1).",".


							renderTimeFormattedSTD($t_ready).",".renderTimeFormattedSTD($t_queue).",".renderTimeFormattedSTD($t_paused).",".round($total_paid/60,2).",\r\n";
