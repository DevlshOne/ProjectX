...
//echo "Caught Exception ".$e->getMessage()."\n";
        ...
        function connect() {
            ...
            $failover = false;
            # CREATE MYSQL DATABASE CONNECTION
            $this->db = mysqli_connect($_SESSION['site_config']['pxdb_lb'][$mod]['sqlhost'], $_SESSION['site_config']['pxdb_lb'][$mod]['sqllogin'], $_SESSION['site_config']['pxdb_lb'][$mod]['sqlpass'], $_SESSION['site_config']['pxdb_lb'][$mod]['sqldb']) or $failover = true;
        ...
        function connectReadSlave() {
            ...
            $failover = false;
            # CREATE MYSQL DATABASE CONNECTION
            $this->db_readslave = mysqli_connect($_SESSION['site_config']['pxdb_readonly'][$mod]['sqlhost'], $_SESSION['site_config']['pxdb_readonly'][$mod]['sqllogin'], $_SESSION['site_config']['pxdb_readonly'][$mod]['sqlpass'], $_SESSION['site_config']['pxdb_readonly'][$mod]['sqldb']) or $failover = true;
...
// 		print_r($status);
        ...
        function execSQL($cmd, $ignore_error = false, $no_debug = false) {
            ...
            if (($cnt = mysqli_affected_rows($this->db)) > 0) return $cnt; else
                return 0;
        }
        ...
        function queryROWS($cmd) {
            return $this->query($cmd, 5);
        }    # Returns the number of rows in a result set

        function fetchROW($cmd) {
        ...
        function ROqueryROW($cmd) {
            return $this->ROQuery($cmd, 2);
        }    # Returns an array of 1 result

        function ROqueryOBJ($cmd) {
            return $this->ROQuery($cmd, 3);
        }    # Returns an object of first result

        function ROquerySQL($cmd) {
            return $this->ROQuery($cmd, 4);
        }    # Returns as associative-array(hash) of 1 result

        function ROqueryROWS($cmd) {
            return $this->ROQuery($cmd, 5);
        }    # Returns the number of rows in a result set

        function ROfetchROW($cmd) {
            return $this->ROQuery($cmd, 6);
        }    # Returns an associative array that corresponds to the fetched row, or FALSE if there are no more rows.
